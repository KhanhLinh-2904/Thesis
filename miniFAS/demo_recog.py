...
def verify(frame_verify, result_verify):
    while True:
        # Get frame from the queue
        images = frame_verify.get()
        if len(images) < 2:
            continue
        elif len(images) == 2:
            ...
            if similarity >= 0.75:
                result_verify.put("MATCH ({})".format(similarity))
            else:
                result_verify.put("NOT_MATCH ({})".format(similarity))
...
def camera(frame_fas, result_fas, frame_verify, result_verify):
    ...
    while cap.isOpened():
        ...
        # cv2.imshow("card_frameName", card_frame)
...
def anti_spoofing(frame_fas, result_fas):
    while True:
        real, fake = 0, 0

        # Get frame from the queue
        detections = frame_fas.get()
